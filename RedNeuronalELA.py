...
def preprocessing(image_path, target_size=(128, 128), ela_scale=90):
    # Cargar la imagen y redimensionarla
    original_image = cv2.imread(image_path)
    original_image = cv2.cvtColor(original_image, cv2.COLOR_BGR2RGB)
    original_image_resized = cv2.resize(original_image, target_size)

    # Guardar la imagen redimensionada temporalmente y luego volver a cargarla para aplicar ELA
    temp_path = "temp_resized_image.jpg"
    cv2.imwrite(temp_path, cv2.cvtColor(original_image_resized, cv2.COLOR_RGB2BGR))
    ela_image = cv2.imread(temp_path)

    # Aplicar ELA
    ela_image = cv2.cvtColor(ela_image, cv2.COLOR_BGR2GRAY)
    ela_image = cv2.resize(ela_image, target_size)
    ela_image = cv2.absdiff(ela_image, cv2.medianBlur(ela_image, 3))

    # Redimensionar la imagen ELA para que coincida con el tamaño de destino
    ela_image_resized = cv2.resize(ela_image, target_size)

    # Normalizar los valores de píxeles después de ELA
    ela_image_resized = ela_image_resized.astype(np.float32)
    ela_image_resized = (ela_image_resized - ela_image_resized.min()) / (ela_image_resized.max() - ela_image_resized.min())

    #mostrar_imagenes(original_image, ela_image_resized)

    # Eliminar la imagen temporal
    os.remove(temp_path)
    return ela_image_resized.flatten()
...
model.save("imageforgerymodelELA.keras")

laodedmodel = load_model("imageforgerymodelELA.keras")
laodedmodel.summary()