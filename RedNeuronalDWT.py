...
def preprocessing(image_path, target_size=(128, 128)):
    ...
    # Aplicar DWT a la imagen
    coeffs = pywt.dwt2(original_image_resized, 'haar')
    LL, (LH, HL, HH) = coeffs

    # Normalizar los valores de píxeles de la aproximación
    LL_resized = (LL - LL.min()) / (LL.max() - LL.min())

    #mostrar_imagenes(original_image, LL_resized.reshape(128, 128, -1))

    return LL_resized.flatten()
